...
#Recta de Regresión:
ggplot(paisesCat, aes(Alfabet, Esp.vida.Fem)) + ggtitle("Recta de Regresión de los países mayoritariamente Católicos:") + xlab("Alfabetización") + ylab("Esperanza de Vida Femenina") + geom_point(colour = "gold") + geom_smooth(method = "lm", se = FALSE, colour = "black")
...
plot(paises.8$Alfabet, paises.8$Esp.vida.Fem, main = "Comparativa entre todos los países(rosa) y los Católicos (dorada):", xlab = "Alfabetización", ylab =  "Esperanza de Vida Femenina", col = "red")
abline(lm(paises.8$Esp.vida.Fem ~ paises.8$Alfabet), col = "pink")
abline(lm(paisesCat$Esp.vida.Fem ~ paisesCat$Alfabet), col = "gold")

#Como se puede ver en el gráfico superior los países católicos en media tienen mayor
#alfabetización y la pendiente de su recta es más inclinada llegando así a representar
#mejor a los valores más altos de la esperanza de vida femenina.
